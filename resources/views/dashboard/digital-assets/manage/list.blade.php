    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h1 class="card-title mb-0" style="width:50%;float:left">Asset List</h1>
                <!-- Add Button -->
                @can('Add Template')
                    <a href="{{ url('digital-assets/add') }}" class="btn btn-primary btn-sm" style="float:right;">Add Digital Asset</a>
                @endcan
            </div>
            <div class="card-body">
                <table id="example" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">Exchangeable</th>
                            <th class="text-center">Giftable</th>
                            <th class="text-center">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($assets as $serial => $asset)
                            <tr>
                                <td class="text-center">{{ $asset->name }}</td>
                                <td style="white-space: normal;" class="text-center"><img width="50px" src="{{ url($asset->image)  }}"></td>
                                <td style="white-space: normal;" class="text-center">{{$asset->quantity.' / $'.$asset->price}}</td>
                                <td style="white-space: normal;" class="text-center">{{$asset->exchangeable ? 'Yes' : 'No'}}</td>
                                <td style="white-space: normal;" class="text-center">{{$asset->giftable ? 'Yes' : 'No'}}</td>
                                <!-- Action Button -->
                                        ...
                                        @canany(['Edit Template', 'Delete Template', 'Preview Template'])
                                            <div class="dropdown d-inline-block">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    ...
                                                    @can('Edit Template')
                                                        <li>
                                                            <a href="" class="dropdown-item edit-item-btn"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</a>
                                                        </li>
                                                    @endcan
                                                    <!-- Delete Button -->
                                                    @can('Delete Template')
                                                        <li>
                                                             <a type="button" class="dropdown-item remove-item-btn" onclick=""><i class="ri-delete-bin-fill align-bottom me-2 text-muted"></i> Delete</a>
                                                        </li>
                                                    @endcan
                                                </ul>
                                            </div>
