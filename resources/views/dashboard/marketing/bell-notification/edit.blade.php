    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h1 class="card-title mb-0">Edit Notification</h1>
            </div>
            <div class="card-body">
                <div class="row">
                    <form method="POST" action="{{ url('/marketing/bell-notifications/update',$bellNotification->id) }}" enctype="multipart/form-data">
                        @csrf
                        <div class="row">

                            <div class="col-md-12 form-group mb-3">
                                <label for="notification-body" class="form-label">Notification Body</label>
                                <textarea name="message_body" class="form-control"  rows="6" id="notification-body" placeholder="Notification Body">{{$bellNotification->messages}}</textarea>
                            </div>
                            <div class="col-md-4 form-group mb-3">
                                <label for="duration" class="form-label">Duration Day</label>
                                <input type="text" class="form-control datepicker-here" value="{{ \Carbon\Carbon::parse($bellNotification->duration)->format('d.m.Y')}}" name="duration_day" id="myDatePicker">
                            </div>
                            <div class="col-md-4 form-group mb-3">
                                <label for="myTimePicker" class="form-label">Duration Time</label>
                                <input type="text" class="form-control timepicker" value="{{ \Carbon\Carbon::parse($bellNotification->duration)->format('H:i:s')}}" name="duration_time" id="myTimePicker">
                            </div>
